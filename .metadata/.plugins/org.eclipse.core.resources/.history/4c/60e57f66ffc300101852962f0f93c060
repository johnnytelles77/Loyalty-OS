...
public class UserController {
    ...
    @Operation(summary = "Create a new User or return existing by phone")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "User created successfully",
                content = @Content(schema = @Schema(implementation = UserDTO.class))),
        @ApiResponse(responseCode = "200", description = "User already exists, returned",
                content = @Content(schema = @Schema(implementation = UserDTO.class))),
        @ApiResponse(responseCode = "500", description = "Internal Server Error")
    })
    @PostMapping
    ...
    public ResponseEntity<UserDTO> getUserByTelefono(@PathVariable String telefono) {
        try {
            User user = userService.getUserByTelefonoExact(telefono);
